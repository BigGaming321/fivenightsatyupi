...
void DoorManager::_ready() {
    ...
    if (!root) { UtilityFunctions::printerr("DoorManager: no parent found!"); return; }

    camera_manager = root->get_node<CameraManager>("CameraManager");
    if (!camera_manager) { UtilityFunctions::printerr("DoorManager: CameraManager not found!"); return; }
    ...
    if (!left_door.display)  { UtilityFunctions::printerr("DoorManager: LeftDoor not found!"); return; }
    if (!right_door.display) { UtilityFunctions::printerr("DoorManager: RightDoor not found!"); return; }
...
void DoorManager::set_power_out(bool value) {
    ...
    if (value) {
        ...
        if (left_door_button)   left_door_button->hide();
        if (right_door_button)  right_door_button->hide();
        if (lights_button)      lights_button->hide();
    } else {

        if (left_door_button)   left_door_button->show();
        if (right_door_button)  right_door_button->show();
        if (lights_button)      lights_button->show();
    }
}
